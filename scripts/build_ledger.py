...
def main() -> None:
    ...
    # Optional: local governance scope config (recommended)
    scope_path = cfg.get("governance_scope_config", "config/governance_scope.json")
    layers = load_scope_layers(scope_path) if os.path.exists(scope_path) else {}
    ...
    if not salt:
        raise SystemExit(f"Variable d’environnement manquante : {salt_env}")
    ...
    rows = load_rows(
        input_cfg["csv_path"],
        cols["timestamp"],
        cols.get("ip") or cols.get("ip_hash") or "ClientIP",
        cols["ua"],
        cols["path"],
        cols["host"],
        cols.get("method"),
    ...
    for r in rows:
        if r.method and allow_methods and r.method.upper() not in [m.upper() for m in allow_methods]:
            continue
        if r.status and allow_status and r.status not in [str(s) for s in allow_status]:
            continue
        # If governance paths list is present, keep only those
        if gov_paths and r.path not in set(gov_paths):
            continue
    ...
    state_path = "state/ledger-state.json"
    ...
    ledger: Dict[str, Any] = {
        ...
        "method": {
            ...
            "notes": "sessions inferees a partir de logs; aucune identite de modele n'est affirmee"
        },
    ...
    with open(cfg["output"]["yaml"], "w", encoding="utf-8") as f:
        f.write("# Fichier miroir lisible\n")
        f.write("# Canonique : /.well-known/q-ledger.json\n")
        f.write("# Ne pas editer manuellement lorsque le pipeline est actif\n\n")
    ...
    print(f"OK: sessions={len(sessions)} | seq={ledger_sequence} | hash={ledger_hash[:12]}…")
