...
# Always use the same input file for B
$SourceCsv = "input\cloudflare_logs.csv"
$OutDir = "exports\B-full-governance"
...
$dstCsv = Join-Path $OutDir ("cloudflare-full-governance-" + $DateUtc + ".csv")
...
$summary = [ordered]@{
  type = "cloudflare_full_governance_capture"
  date_utc = $DateUtc
...
$dstJson = Join-Path $OutDir ("summary-full-governance-" + $DateUtc + ".json")
($summary | ConvertTo-Json -Depth 6) | Set-Content -LiteralPath $dstJson -Encoding UTF8
Ok "Wrote JSON: $dstJson"

$dstMd = Join-Path $OutDir ("summary-full-governance-" + $DateUtc + ".md")

$md = @()
$md += "# Cloudflare capture - full governance"
$md += ""
$md += ("- Date (UTC): " + $DateUtc)
$md += ("- Rows: " + $rows.Count)
...
Info ""
Info "Done. B series captured from input\cloudflare_logs.csv and stored under exports\B-full-governance."
Info ""
